...
function loadDataTable() {
    dataTable = $('#tblData').DataTable({
        "ajax": { url: '/Admin/CheDoPhucLoi/GetAll' },
        "columns": [
            { data: 'name', width: '50%'},
            {
                data: 'isApplied',
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    return `<div class="w-100 btn-group ms-auto btn-group-equal" role="group">
                                <a href="/Admin/CheDoPhucLoi/Upsert?id=${data}" class="btn btn-primary mx-2"><i class="bi bi-pencil-square"></i> Sửa</a>
                                <a onClick="Delete('/Admin/CheDoPhucLoi/Delete/${data}')" class="btn btn-danger mx-2"><i class="bi bi-trash-fill"></i> Xóa</a>
...
function Delete(url) {
    Swal.fire({
        title: "Bạn chắc chứ?",
        text: "Danh mục chế độ phúc lợi sẽ bị xóa vĩnh viễn!",
        icon: "warning",
...
function ToggleApply(id, isApplied) {
    Swal.fire({
        ...
        text: isApplied ? "Chế độ phúc lợi sẽ ngừng áp dụng." : "Chế độ phúc lợi sẽ được áp dụng.",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/Admin/CheDoPhucLoi/ToggleApply',
                type: 'POST',
                data: { id: id },
                success: function (data) {
                    dataTable.ajax.reload();
